...
async function run() {
    ...
    try {
        ...
        app.get('/services', async (req, res) => {
            const query = {};
            const services = await serviceCollection.find(query).limit(3).sort({ _id: -1 }).toArray();
            res.send(services);
        })
        ...
        app.get('/allServices', async (req, res) => {
            const query = {}
            const services = await serviceCollection.find(query).sort({ _id: -1 }).toArray();
